...
let modifyElement = (el, host, index) => {
	...
	if(typeof el.tagName === 'function') {
		let run = props => el.tagName(props, nextProps => render(run(Object.assign(el.props, nextProps)), host, index))
...
let render = exports.render = (el, host, index = 0) => {
	...
	el.children.forEach((child, i) => render(child, extantChild || node, i))
